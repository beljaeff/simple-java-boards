...
public class PostController extends AbstractController {
    ...
    @GetMapping(RoutesHelper.POST_REPLY)
    ...
    @GetMapping(RoutesHelper.POST_EDIT)
    ...
    @PreAuthorize("hasAnyPermissions('CREATE_POST', 'ADMIN')")
    @PostMapping(RoutesHelper.POST_SAVE_REPLY)
    ...
    @PreAuthorize("hasAnyPermissions('EDIT_POST', 'EDIT_OWN_POST', 'ADMIN')")
    @PostMapping(RoutesHelper.POST_SAVE)
    ...
    protected void setFormUrls(BaseForm baseForm) {
        ...
        if(form.getId() == 0) {
            ...
            if(form.getIdParent() != null) {
                form.setFormUrl(buildSegmentsQueryPath(RoutesHelper.POST_REPLY, RoutesHelper.PAGE_QUERY, Map.of(BaseController.ATTR_ID, form.getIdParent(), BaseController.ATTR_PAGE, form.getFromPage())));
            }
            // Add from topics list
            else {
                form.setFormUrl(buildSegmentsQueryPath(RoutesHelper.TOPIC_VIEW, RoutesHelper.PAGE_QUERY, Map.of(BaseController.ATTR_ID, form.getIdTopic(), BaseController.ATTR_PAGE, form.getFromPage())));
            }
            form.setSaveUrl(RoutesHelper.POST_SAVE_REPLY);
        }
        // Edit post
        else {
            form.setFormUrl(buildSegmentsQueryPath(RoutesHelper.POST_EDIT, RoutesHelper.PAGE_QUERY, Map.of(BaseController.ATTR_ID, form.getId(), BaseController.ATTR_PAGE, form.getFromPage())));
            form.setSaveUrl(RoutesHelper.POST_SAVE);
        }
        form.setCancelUrl(buildSegmentsQueryPath(RoutesHelper.TOPIC_VIEW, RoutesHelper.PAGE_QUERY, Map.of(BaseController.ATTR_ID, form.getIdTopic(), BaseController.ATTR_PAGE, form.getFromPage())));
        form.setFinishUrl(buildSegmentsQueryPath(RoutesHelper.TOPIC_VIEW, RoutesHelper.PAGE_QUERY, Map.of(BaseController.ATTR_ID, form.getIdTopic(), BaseController.ATTR_PAGE, form.getToPage())));
    }

    @PreAuthorize("hasAnyPermissions('CREATE_POST', 'ADMIN')")
    @GetMapping(RoutesHelper.POST_SAVE_REPLY)
    public String redirectReply(Model model) {
        return super.redirect(model);
    }

    @PreAuthorize("hasAnyPermissions('EDIT_POST', 'EDIT_OWN_POST', 'ADMIN')")
    @GetMapping(RoutesHelper.POST_SAVE)
    public String redirectEdit(Model model) {
        return super.redirect(model);
    }

    @PreAuthorize("hasAnyPermissions('DELETE_POST', 'ADMIN')")
    @GetMapping(RoutesHelper.POST_DELETE)
    public String delete(@PathVariable int id, @RequestParam(required = false) Integer page) {
        return "redirect:" + makeLinkForRedirect(RoutesHelper.TOPIC_VIEW, page, postService.delete(id));
    }

    @PreAuthorize("hasAnyPermissions('EDIT_POST', 'ACTIVATE_POST', 'ADMIN')")
    @GetMapping(RoutesHelper.POST_CHANGE_ACTIVE)
    public String changeActive(@PathVariable int id, @RequestParam(required = false) Integer page) {
        return "redirect:" + makeLinkForRedirect(RoutesHelper.TOPIC_VIEW, page, postService.changeActive(id));
    }
    ...
    private String makeLinkForRedirect(String address, Integer page, ActionStatusDto<Post> actionStatus) {
        ...
        if(page != null) {
            ...
            return buildSegmentsQueryPath(address, RoutesHelper.PAGE_QUERY, uriParams);
        }
        else {
